...
public class CalendarPickerView extends ListView {
  ...
  final List<MonthCellDescriptor> bookedCells = new ArrayList<>();
  final List<Calendar> selectedCals = new ArrayList<>();
  final List<Calendar> highlightedCals = new ArrayList<>();
  final List<Calendar> bookedCals = new ArrayList<>();
  ...
  public class FluentInitializer {
    ...
    public FluentInitializer withBookedDates(Collection<Date> dates) {
      bookedDates(dates);
      return this;
    }

    public FluentInitializer withBookedDate(Date date) {
      return withBookedDates(Collections.singletonList(date));
    }
  ...
  public void bookedDates(Collection<Date> dates) {
    for (Date date : dates) {
      validateDate(date);
      ...
      if (monthCellWithMonthIndex != null) {
        ...
        bookedCells.add(cell);
        bookedCals.add(newlyHighlightedCal);
        cell.setBooked(true);
      }
    }
  ...
  public void clearBookedDates() {
    for (MonthCellDescriptor cal : bookedCells) {
      cal.setHighlighted(false);
    }
    bookedCells.clear();
    bookedCals.clear();
  ...
  List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
    ...
    while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
      ...
      for (int c = 0; c < 7; c++) {
        ...
        boolean isBooked = containsDate(bookedCals, cal);
        ...
        weekCells.add(
            new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday,
                isHighlighted, value, rangeState, isBooked));
        cal.add(DATE, 1);
      }
    }
    return cells;
