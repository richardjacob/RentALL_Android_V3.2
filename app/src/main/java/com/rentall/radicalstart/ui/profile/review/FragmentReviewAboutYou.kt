...
class FragmentReviewAboutYou : BaseFragment<FragmentReviewBinding,ReviewViewModel>(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        pagingController = ReviewListController(this.requireContext(),"aboutYou")
        subscribeToLiveData()
        viewModel.networkStateAboutYou.observe(this, Observer {
            it?.let { networkState ->
                when (networkState) {
    ...
    fun subscribeToLiveData(){
        viewModel.loadAboutYouList().observe(this, Observer<PagedList<GetUserReviewsQuery.Result>>{
            it?.let {
