...
class SavedFragment : BaseFragment<FragmentSavedBinding, SavedViewModel>(), SavedNavigator {
    ...
    private fun subscribeToLiveData() {
        viewModel.loadwishListGroup().observe(this, Observer<PagedList<GetAllWishListGroupQuery.Result>> { pagedList ->
            pagedList?.let {
...
//                        pagingController.isLoading = true
    ...
    override fun onRetry() {
        if(::mViewModelFactory.isInitialized) {
            ...
            if (count == 1) {
                val fragment = childFragmentManager.findFragmentByTag("SavedDetails") as SavedDetailFragment
                fragment.onRetry()
            } else {
            ...
            try{
                return ViewholderSavedListItemsBindingModel_()
                        ...
                        .clickListener(View.OnClickListener {
                            try {
                                openFragment(SavedDetailFragment.newInstance(item.id()!!, item.name()!!.trim().replace("\\s+", " "), item.wishListCount()!!, viewModel.wishlistType!!), "SavedDetails")
                            } catch (e: Exception) {
                                e.printStackTrace()
                                showError()
                            }
